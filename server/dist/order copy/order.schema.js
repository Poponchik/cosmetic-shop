"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.OrderSchema = exports.Order = void 0;
const mongoose_1 = require("@nestjs/mongoose");
const mongoose = require("mongoose");
const user_schema_1 = require("../users/user.schema");
const swagger_1 = require("@nestjs/swagger");
const productsInOrder_schema_1 = require("./productsInOrder.schema");
let Order = class Order {
};
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'id юзера' }),
    (0, mongoose_1.Prop)({ type: mongoose.Schema.Types.ObjectId, ref: () => user_schema_1.User }),
    __metadata("design:type", mongoose.Schema.Types.ObjectId)
], Order.prototype, "userId", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'email' }),
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Order.prototype, "email", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'номер заказа' }),
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Order.prototype, "numberPhone", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'статус', description: 'выполнено' }),
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Order.prototype, "status", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'адрес доставки', description: 'Чистяковская 15' }),
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", String)
], Order.prototype, "address", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'общая сумма заказа' }),
    (0, mongoose_1.Prop)({ required: true }),
    __metadata("design:type", Number)
], Order.prototype, "totalPrice", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'дата заказа', description: '15.10.2022' }),
    (0, mongoose_1.Prop)({ type: Date, default: Date.now }),
    __metadata("design:type", String)
], Order.prototype, "timestamp", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 'массив id товаров и колиество' }),
    (0, mongoose_1.Prop)([productsInOrder_schema_1.ProductsInOrder]),
    __metadata("design:type", Array)
], Order.prototype, "products", void 0);
Order = __decorate([
    (0, mongoose_1.Schema)()
], Order);
exports.Order = Order;
exports.OrderSchema = mongoose_1.SchemaFactory.createForClass(Order);
//# sourceMappingURL=order.schema.js.map